...
//            RemoteVersion[] versions = service.getVersions(Token, issue.ProjectKey);
        ...
        public void runIssueActionWithParams(JiraIssue issue, int id, ICollection<JiraField> fields, string comment) {
            ...
            service.progressWorkflowAction(Token, issue.Key, id.ToString(), (Atlassian.plvs.Atlassian.plvs.api.soap.service.RemoteFieldValue[])fieldValues.ToArray());
            if (comment != null) {
                addComment(issue, comment);
            }//Error	9	Argument '4': cannot convert from 'object[]' to 'Atlassian.plvs.Atlassian.plvs.api.soap.service.RemoteFieldValue[]'	C:\work\c#\JIRA_VS_Connector\plvs\plvs\api\jira\soap\SoapSession.cs	305	77	plvs2008
